...
- Spark Operator installed on the cluster (via Kustomize: `oc apply -k config/default/ --server-side=true`)
...
```bash
oc apply -f examples/openshift/k8s/kueue/kueue-cr.yaml
```
...
```bash
oc apply -f examples/openshift/k8s/kueue/kueue-resources.yaml
```
...
```bash
oc apply -f examples/openshift/k8s/kueue/../base/rbac.yaml
```

## Step 7: Submit a SparkApplication

```bash
oc apply -f examples/openshift/k8s/kueue/spark-pi-kueue.yaml
```
...
Automated Ginkgo tests are in `examples/openshift/tests/kueue_test.go`. They cover:
...
go test -v -tags openshift ./examples/openshift/tests/ \
  -ginkgo.v \
  -ginkgo.focus="Kueue" \
  -timeout 35m
```
...
|----------|----------|---------|
| `SKIP_OPERATOR_INSTALL=true` | Yes | Skips Helm install of the Spark Operator (you already have it deployed) |
| `SKIP_NAMESPACE_CLEANUP=true` | Yes | Prevents `AfterSuite` from deleting the `spark-operator` namespace |
| `KUBECONFIG` | Yes | Path to your kubeconfig for the OpenShift cluster |
