...
To modify the pipelines for `spark-operator` in the `main` branch:
...
```bash
cd pipelineruns/spark-operator/.tekton
```
...
```bash
git commit -am "Update pipelinerun for spark-operator (main)"
